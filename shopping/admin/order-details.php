...
if(isset($_POST['takeaction']))
{
    ...
    if($status=='Cancelled'):
   $query="insert into ".PREFIX."ordertrackhistory(orderId,status,remark,actionBy,canceledBy) values('$oid','$status','$remark','$actionby',' $canceledBy');";
   $query.="update ".PREFIX."orders set orderStatus='$status' where id='$oid'";
else:
  $query="insert into ".PREFIX."ordertrackhistory(orderId,status,remark,actionBy) values('$oid','$status','$remark','$actionby');";
   $query.="update ".PREFIX."orders set orderStatus='$status' where id='$oid'";
endif;    
...
<?php 
$oid=$_GET['orderid'];
echo "SELECT ".PREFIX."orders.id,orderNumber,totalAmount,orderStatus,orderDate,".PREFIX."users.contactno,".PREFIX."orders.txnType,".PREFIX."orders.txnNumber
,".PREFIX."users.name,".PREFIX."users.email,".PREFIX."users.contactno,billingAddress,biilingCity,billingState,billingPincode,billingCountry,shippingAddress,shippingCity,shippingState,shippingPincode,shippingCountry
    FROM `".PREFIX."orders` join ".PREFIX."users on ".PREFIX."users.id=".PREFIX."orders.userId 
join ".PREFIX."addresses on ".PREFIX."addresses.id=".PREFIX."orders.addressId
    where (".PREFIX."orders.id='$oid')";die;
$query=mysqli_query($con,"SELECT ".PREFIX."orders.id,orderNumber,totalAmount,orderStatus,orderDate,".PREFIX."users.contactno,".PREFIX."orders.txnType,".PREFIX."orders.txnNumber
,".PREFIX."users.name,".PREFIX."users.email,".PREFIX."users.contactno,billingAddress,biilingCity,billingState,billingPincode,billingCountry,shippingAddress,shippingCity,shippingState,shippingPincode,shippingCountry
    FROM `".PREFIX."orders` join ".PREFIX."users on ".PREFIX."users.id=".PREFIX."orders.userId 
join ".PREFIX."addresses on ".PREFIX."addresses.id=".PREFIX."orders.addressId
    where (".PREFIX."orders.id='$oid')");
$cnt=1;
...
<?php 
$oid=$_GET['orderid'];
$query=mysqli_query($con,"SELECT ".PREFIX."products.id as pid,".PREFIX."products.productName,".PREFIX."products.productImage1,products.productPrice,".PREFIX."products.shippingCharge, ".PREFIX."ordersdetails.quantity FROM `".PREFIX."ordersdetails` 
JOIN ".PREFIX."orders on ".PREFIX."orders.orderNumber=".PREFIX."ordersdetails.orderNumber
join ".PREFIX."products on ".PREFIX."products.id=".PREFIX."ordersdetails.productId
    where (".PREFIX."orders.id='$oid')");
$cnt=1;
while($row=mysqli_fetch_array($query))
{
