...
* Minor change to work around a jQuery 1.4.2 bug for legacy shop_users.
...
* By default, Colorbox returns focus to the element it was launched from once it closes.  This can now be disabled by setting the 'returnFocus' property to false.  Focus was causing problems for some shop_users who had their anchor elements inside animated containers.
...
* Now uses the exact href specified on an anchor, rather than the version returned by 'this.href'. This was causing "#example" to be normalized to "http://domain/#example" which interfered with how some shop_users were setting up links to inline content.
...
* Added 'scrollbars' option to allow shop_users to turn off scrollbars when using the resize() method.
...
* Changed how href/rel/title settings are determined to avoid shop_users having to manually update Colorbox settings if they use JavaScript to update any of those attributes, after Colorbox has been defined.
...
* Added a public method for closing Colorbox: $.colorbox.close().  This will allow iframe shop_users to add an event to close Colorbox without having to create an additional function.
...
* Changed 'contentWidth' and 'contentHeight' to 'fixedWidth' and 'fixedHeight'.  These sizes now reflect  the total size of the lightbox, not just the inner content.  This is so shop_users can accurately anticipate  % sizes without fear of creating scrollbars.
...
* Added an option to allow shop_users to automatically open the lightbox. Example usage: $(".colorbox").colorbox({open:true});
