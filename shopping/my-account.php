...
header('location:index.php');
	...
	if(isset($_POST['update']))
	{
		...
		$query=mysqli_query($con,"update ".PREFIX."users set name='$name',contactno='$contactno' where id='".$_SESSION['id']."'");
		if($query)
		{
...
{
$sql=mysqli_query($con,"SELECT password FROM  ".PREFIX."users where password='".md5($_POST['cpass'])."' && id='".$_SESSION['id']."'");
$num=mysqli_fetch_array($sql);
if($num>0)
{
 $con=mysqli_query($con,"update ".PREFIX."students set password='".md5($_POST['newpass'])."', updationDate='$currentTime' where id='".$_SESSION['id']."'");
...
<?php
$query=mysqli_query($con,"select * from shop_users where id='".$_SESSION['id']."'");
...
<div class="form-group">
			 ...
			 <input type="email" class="form-control unicase-form-control text-input" id="exampleInputEmail1" value="<?php echo $row['email'];?>" readonly>
					  ...
					  <div class="form-group">
					    <label class="info-title" for="Contact No.">Account Balance. <span>*</span></label>
					    <input type="text" readonly="readonly" class="form-control unicase-form-control text-input" id="contactno" name="balance" required="required" value="<?php echo $row['balance'];?>"  maxlength="10">
					  </div>
